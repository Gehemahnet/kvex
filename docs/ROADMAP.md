...
- current Funding view is still a placeholder
...
- funding service aggregates per-exchange data from `hyperliquid`, `pacifica`, and `ethereal`
...
- `exchanges`: optional comma-separated list, currently `hyperliquid`, `pacifica`, `ethereal`
...
1. Stabilize Funding-only frontend shell.
2. Build the Funding page around the `/funding` query contract.
3. Keep Vite devtools enabled for local inspection.
4. Finish backend contract for spreads.
5. Add normalized market snapshot model.
6. Add Redis for hot state and pubsub.
7. Add spread/opportunity engine with confidence scoring.
8. Add Postgres/Timescale for history.
9. Add portfolio ingestion and secrets model.
